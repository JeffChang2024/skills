...
2. **不要退出登录**：导出 Cookie 后不要在浏览器中退出小红书登录。
3. **不要清理浏览器数据**：清理 Cookie 会导致导出的 Cookie 立即失效。
4. **关键字段**：`a1`、`web_session` 和 `webId` 是必需的，缺少任何一个都会导致操作失败。
5. **活跃续期**：建议在 SocialVault 中开启活跃续期功能，自动访问小红书页面延长 Cookie 有效期。
6. **多账号**：如果有多个小红书账号，需要分别登录并导出 Cookie。可以使用浏览器的隐身模式或不同浏览器 Profile。
