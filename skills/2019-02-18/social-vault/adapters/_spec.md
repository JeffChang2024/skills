...
官方适配器放在 `adapters/` 目录下，用户自建适配器放在 `adapters/custom/` 目录下。文件名格式为 `平台标识.md`，如 `reddit.md`、`v2ex.md`。

## 三、Frontmatter 必填字段

| 字段 | 类型 | 说明 | 示例 |
|------|------|------|------|
| platform_id | string | 唯一标识，小写字母和连字符 | `reddit` |
| platform_name | string | 展示名称 | `Reddit` |
