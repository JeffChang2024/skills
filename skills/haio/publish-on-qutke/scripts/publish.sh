...
BASE_URL="https://on.qutke.cn"
CREDENTIALS_FILE="$HOME/.onqutke/credentials"
API_KEY="${ONQUTKE_API_KEY:-}"
API_KEY_SOURCE="none"
if [[ -n "${ONQUTKE_API_KEY:-}" ]]; then
  API_KEY_SOURCE="env"
fi
ALLOW_NON_ONQUTKE_BASE_URL=0
SLUG=""
CLAIM_TOKEN=""
TITLE=""
DESCRIPTION=""
...
Options:
  --api-key <key>         API key (or set $ONQUTKE_API_KEY)
  ...
  --base-url <url>        API base (default: https://on.qutke.cn)
  --allow-nononqutke-base-url
...
if [[ -x "$BUNDLED_JQ" ]]; then
  ...
  die "requires jq (run: curl -fsSL https://on.qutke.cn/install.sh | bash)"
fi
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --allow-nononqutke-base-url) ALLOW_NON_ONQUTKE_BASE_URL=1; shift ;;
    --help|-h)      usage ;;
...
if [[ -n "$API_KEY" && "$BASE_URL" != "https://on.qutke.cn" && "$ALLOW_NON_ONQUTKE_BASE_URL" -ne 1 ]]; then
  die "refusing to send API key to non-default base URL; pass --allow-nononqutke-base-url to override"
fi
...
CLIENT_HEADER_VALUE="on-qutke-publish-sh"
if [[ -n "$CLIENT" ]]; then
...
CLIENT_ARGS=(-H "x-onqutke-client: $CLIENT_HEADER_VALUE")
