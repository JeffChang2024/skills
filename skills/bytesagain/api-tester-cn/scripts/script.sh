#!/usr/bin/env bash
# api-tester-cn — description: "API请求构造、curl命令生成、Mock数据、API文档、HTTP状态码速查、Headers说明。API request buil
# Powered by BytesAgain | bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${API_TESTER_CN_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/api-tester-cn}"
...
cmd_init() {
    ensure_dirs
    echo "[api-tester-cn] Initialized at $DATA_DIR"
    echo "  Entries file: $ENTRIES"
    echo "  Config file:  $CONFIG"
    echo ""
    echo "Ready. Try: api-tester-cn add \"your first item\""
}

cmd_add() {
    ensure_dirs
    local text="${*:?Usage: api-tester-cn add <text>}"
    local ts=$(now_ts)
    local id=$(($(entry_count) + 1))
    printf '{"id":%d,"text":"%s","created":"%s"}\n' "$id" "$text" "$ts" >> "$ENTRIES"
    echo "[api-tester-cn] #$id added: $text"
}
...
cmd_list() {
    ...
    if [ "$total" -eq 0 ]; then
        echo "[api-tester-cn] No entries yet. Use: api-tester-cn add <text>"
        return
    fi
    echo "[api-tester-cn] $total entries:"
    echo ""
    local n=0
...
cmd_remove() {
    ensure_dirs
    local id="${1:?Usage: api-tester-cn remove <id>}"
    local total=$(entry_count)
    if [ "$id" -lt 1 ] || [ "$id" -gt "$total" ]; then
        echo "Error: id must be 1-$total"; return 1
    fi
    ...
    echo "[api-tester-cn] Removed #$id: $text"
}
...
cmd_search() {
    ...
    echo "[api-tester-cn] Found $found matches for '$term'"
}
...
cmd_config() {
    ensure_dirs
    echo "[api-tester-cn] Configuration"
    echo "  File: $CONFIG"
    echo ""
    python3 -c "
import json
...
cmd_status() {
    ensure_dirs
    echo "[api-tester-cn] Status Check"
...
cmd_reset() {
    ...
    if [ "$confirm" = "yes" ]; then
        rm -f "$ENTRIES" "$CONFIG"
        echo "[api-tester-cn] Data cleared."
    else
        echo "Cancelled."
    fi
}
...
case "${1:-help}" in
    ...
    version|-v)     echo "api-tester-cn v$VERSION" ;;
